...
def append_unplaced_footnotes_as_paras(unplaced_footnotes: list[str], xml_file: str) -> str:
    paragraphs = "".join(
        f"<core:para>{footnote}</core:para>"
        for footnote in unplaced_footnotes
...
def add_footnotes(clean_footnotes_list: list[str], xml_file: str, html_file: Path) -> str:
    ...
    if unplaced_fns:
        xml_file = append_unplaced_footnotes_as_paras(unplaced_fns, xml_file)
...
def write_bia_xml(
    ...
    output_file: Path,
    html_file: Path | None,
    clean_footnotes_list: list[str] | None = None,
) -> Path:
    root = xml.Element("ad:decision-grp", volnum=year)
    ...
    if clean_footnotes_list:
        if html_file is not None and html_file.exists() and html_file.is_file():
            xml_string = add_footnotes(clean_footnotes_list, xml_string, html_file)
        else:
            xml_string = append_unplaced_footnotes_as_paras(clean_footnotes_list, xml_string)
