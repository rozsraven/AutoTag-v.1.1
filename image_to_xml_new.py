...
def process_bia_ocr_string(ocr_text: str):
    paragraphs = [line.strip() for line in ocr_text.split("\n") if line.strip()]
    case_name = paragraphs[0] if paragraphs else "caseNameNotFound"
    file_num = next(
        (
            paragraph
            for paragraph in paragraphs
            if "appeal" in paragraph.lower() or re.search(r"\bA[- ]?\d", paragraph, flags=re.IGNORECASE)
        ),
        "fileNumNotFound",
    )
    decision_date = next(
        (paragraph for paragraph in paragraphs if is_date(paragraph, fuzzy=True)),
        "decisionDateNotFound",
    )
    type_of_case = [paragraph for paragraph in paragraphs if paragraph.lower().startswith("form ")]
    core_paras = paragraphs[1:]
    return case_name, file_num, decision_date, type_of_case, core_paras
...
def add_bia_emphasis_to_xml(xml_file: str, html_file: Path | None) -> str:
    ...
    for emphasis in soup.find_all(["i", "em"]):
        content = emphasis.get_text(strip=True)
        if not content:
            continue
        xml_file = xml_file.replace(content, f'<core:emph typestyle="it">{content}</core:emph>', 1)
...
def write_bia_xml(
    ...
    file_num_node = xml.SubElement(adjudication_info, "ad:filenum")
    file_num_node.text = file_num

    adjudicator_info = xml.SubElement(adjudication_info, "ad:adjudicator-info")
    juris = xml.SubElement(adjudicator_info, "core:juris")
    juris_info = xml.SubElement(juris, "lnci:jurisinfo")
    xml.SubElement(juris_info, "lnci:usa")
    xml.SubElement(juris, "core:juris-name").text = "Board of Immigration Appeals"

    dates = xml.SubElement(adjudication_info, "ad:dates")
    xml.SubElement(dates, "ad:decisiondate").text = "Date: " + decision_date.replace("~i~", "").replace("~/i~", "")
    ...
    for para in core_paras:
        xml.SubElement(judgment_body, "core:para", indent="none").text = para.replace("~in0~", "")
...
def process_image_folder(
    ...
    *,
    ...
    if normalized_process_type == "BIA":
        case_name, file_num, decision_date, type_of_case, core_paras = process_bia_ocr_string(raw_body)
